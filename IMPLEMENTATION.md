...
- 修正 PNG 下载中分组标题栏文字垂直未居中的问题：仅在 `export-mode` 下锁定 `gf2-titlebar` 与 `gf2-titlebar-text` 的垂直对齐方式，让 `Skin1`、`他の参考画像`、`武器` 等标题在导出 PNG 中与网页预览保持一致。
- 进一步修正 PNG 下载中文字仍偏移的问题：将详情页主标题栏、图库分组标题栏、武器标题栏与主题色块纳入导出尺寸冻结，避免 html2canvas 在克隆文档中重新计算这些小型文字容器的高度，减少 `キャラ紹介`、`図鑑`、`#2ab0d9` 等文字在 PNG 中未居中的偏差。
- 修正 PNG 标题栏居中样式被导出冻结逻辑覆盖的问题：导出时若元素为标题栏或主题色块，不再恢复原始 `block` 显示，而是分别保持 `flex / inline-flex` 并锁定垂直居中，避免 `キャラ紹介`、`図鑑`、`Skin1`、`武器`、`#2ab0d9` 在 PNG 中继续贴上显示。
- 进一步修正 PNG 小字号文字整体偏下的问题：仅在 `export-mode` 下对标题栏文字、分组标签、按钮文字与主题色码施加轻微向上位移与固定行高补偿，修正 html2canvas 渲染时的字基线下沉，让这些文字在 PNG 中更接近容器正中。
- 将 PNG 小标题对齐修正改为更强制的导出模式布局：不再依赖 `translateY` 微调，而是在 `export-mode` 下对标题栏、角色档案标签与主题色码使用固定行高与 `flex / inline-flex` 垂直居中，减少不同小字号文字在 PNG 中仍然偏上或偏下的不一致。
- 进一步收紧 PNG 文字对齐修正：为 `キャラ紹介`、各分组标题、`武器` 与 `キャラクタープロフィール` 文字本身补上导出标记，并在导出克隆阶段直接冻结这些文字节点的实际高度与行高，避免只对外层容器居中时仍出现字基线偏下的问题。
- 调整 PNG 标题文字的导出策略：撤回“按文字自身测量高度冻结”的方式，改为让标题文字在导出时直接继承父标题栏的固定栏高；同时为 `gf2-titlebar`、`gf2-kicker` 与主题色码设置固定导出高度与行高，减少 PNG 中标题始终贴上显示的问题。
- 继续收紧 PNG 标题文字的导出对齐：导出阶段改为读取标题文字父容器的真实高度，并直接用该高度作为文字节点的固定高度与行高，避免 `100% / inherit` 在 html2canvas 克隆文档中失效，进一步减少标题文字在 PNG 中偏上显示的问题。
- 调整 PNG 标题文字定位策略：不再依赖标题字自身的高度或行高，而是在导出克隆阶段读取文字相对父容器的实际左偏移，并将标题文字绝对定位到父容器 `50%` 高度，通过 `translateY(-50%)` 做垂直居中，进一步减少标题文字在 PNG 中持续贴上显示的问题。
- 已回退 PNG 标题文字“单点绝对定位”方案，改为在导出时按父容器真实高度创建整块文字盒并垂直居中，避免标题文字在 PNG 中因 `translateY` 与字体基线叠加而出现更严重的上贴跑位。
- 已回退本轮针对 PNG 标题文字所做的实验性对齐补丁，这批 `export-mode` 标题栏高度锁定、文字节点冻结与绝对定位尝试未能稳定修复问题，且会放大标题错位；当前已恢复到较稳定的基础导出逻辑，后续若继续处理应改走新的导出方案而不是继续叠补丁。
- PNG 标题文字导出改为新方案：为 `gf2-titlebar-text` 与 `キャラクタープロフィール` 补上导出标记，导出时不再依赖 html2canvas 直接渲染这些原始文字节点，而是在克隆文档中按原始字型样式生成独立 SVG 文字图层覆盖到对应位置，降低小字号日文标题在 PNG 中基线漂移的问题。
- 已回退 PNG 标题文字的 SVG 覆盖导出方案：该方案会导致标题文字在当前导出链路中直接消失，现已恢复原文字节点渲染，避免 PNG 中出现无标题状态。
- 已回退本轮 PNG 标题文字的手动画布方案：该方案在当前页面会导致标题错位加剧，现已恢复到原始导出逻辑，不再对标题文字做额外重绘。
- 调整 `他の参考画像` 的超大图页面规则：当参考图任一边超过 `2500px` 时，不再沿用原本的两倍缩放上限，而是优先按原比例以 `w-full` 方式尽量铺满单栏容器，并同步提高卡片最小高度，提升超宽参考图的可视面积且不改变比例。
- 针对 PNG 下载标题视觉偏下的问题，改为最小视觉补偿方案：仅在 `export-mode` 下将 `gf2-kicker` 与 `gf2-titlebar-text` 轻微上移约 `2px`，并同步微调 `gf2-action` 的上下内距，让返回按钮与区块标题在 PNG 中更接近视觉居中，而不改动页面本身版面。
- 按后续确认结果，将 PNG 标题视觉补偿进一步提高为更强的上移版本：`gf2-kicker` 与 `gf2-titlebar-text` 在 `export-mode` 下改为约 `8px` 的上移，返回按钮上下内距也同步加大偏置，用于快速测试“明显抬高标题”后的 PNG 视觉效果。
- 继续细调 PNG 标题补偿：`キャラクタープロフィール` 对应的 `gf2-kicker` 在 `export-mode` 下再单独提高到更大的上移量，使这块标题能追上其他区块标题的视觉居中效果，不影响其余标题补偿值。
- 继续细调 PNG 视觉补偿：为主题色码色块新增 `gf2-theme-color-chip` 类，并在 `export-mode` 下与其他标题类似做约 `8px` 的上移；同时将 `gf2-kicker` 再进一步抬高，用于修正左上 `キャラクタープロフィール` 与右侧 `#2ab0d9` 两处在 PNG 中仍偏下的问题。
- 调整主题色码的 PNG 补偿层级：主题色块本身恢复原位，不再整体上移；改为新增 `gf2-theme-color-chip-text`，仅在 `export-mode` 下对色块内部的色码文字做约 `8px` 上移，让色块位置保持稳定，同时使 `#2ab0d9` 更接近视觉居中。
- 继续微调 `キャラクタープロフィール` 的 PNG 视觉补偿：将 `gf2-kicker` 在 `export-mode` 下的上移量固定为 `16px`，用于直接观察这块标题在下载 PNG 中的居中接近程度。
- 将 `キャラクタープロフィール` 的 PNG 补偿方式改为框内文字微调：不再整体上移 `gf2-kicker`，改为只在 `export-mode` 下收紧其上内距、加大下内距并固定行高，让文字在外框不位移的前提下往上靠近视觉居中。
- 针对 PNG 中角色主名字与标题区下方细线的位置，新增 `export-mode` 补偿：`gf2-hero-name` 上移 `8px`，`gf2-detail-header::after` 的底部细线同步上提 `8px`，让主名字区整体在下载 PNG 中更接近页面预览的视觉位置。
- 调整标题区细线的 PNG 补偿方式：`gf2-detail-header::after` 不再依赖 `bottom` 偏移，而改为在 `export-mode` 下使用 `transform: translateY(-8px)` 直接上移，避免导出时伪元素定位未跟随偏移的问题。
- 根据下载 PNG 与网页预览的直接对比，已撤回主标题区的 PNG 补偿：`gf2-hero-name` 与 `gf2-detail-header::after` 不再在 `export-mode` 下额外上移，避免角色主名字与标题区下方细线被拉离原本版面；当前仅保留小标题与色码文字的视觉补偿。
- 针对主标题区细线仍压到角色名字的问题，改为在 `export-mode` 下直接增加 `gf2-detail-header` 的底部空间，不再继续移动名字或细线本身，让标题区整体高度更接近网页预览的实际版面。
- 继续修正 PNG 下载中个别立绘仍被拉伸的问题：导出克隆阶段不再区分“放大图”和“普通图”的两套 `img` 尺寸策略，而是统一冻结图片在网页上的实际渲染宽高，并同步写入当前 `aspect-ratio`，让下载 PNG 的第三张立绘等细长图维持与页面一致的比例和留白。
- 根据后续对比截图继续收敛 PNG 导出规则：`図鑑 / Skin` 等图库图片不再在导出时硬锁为固定宽高填满栏位，而是恢复为 `auto + max-width/max-height` 的上限约束，仅保留页面当前显示边界，确保下载 PNG 中图库图继续保持网页里的 `object-contain` 留白与原始比例。
- 继续修正 PNG 下载中“主图被硬塞满框、图库大图又失去两倍显示”的冲突：导出时图片尺寸改为优先读取 `computedStyle.width/height`，避免左侧主图的 `scale(2)` 包围盒被误当作原始尺寸重复放大；同时 `図鑑 / Skin` 中带 `data-export-enlarged` 的大图恢复冻结当前双倍显示尺寸，普通图库图继续保持 `auto + max-width/max-height`，让 PNG 同时保留两倍输出与留白比例。
- 根据后续视觉确认，将“任一边超过 `1500px` 的大图增强显示”从原本偏激进的约 `2x` 收敛为约 `1.5x`：`図鑑 / Skin / 参考图` 的大图显式尺寸上限同步缩小，左侧主图的 `scale` 也从 `2` 调整为 `1.5`，降低 PNG 与网页中细长立绘放大过头、留白失衡的问题。
- 根据最新确认，已关闭“任一边超过 `1500px` 的图片增强显示”规则：`図鑑 / Skin / 参考图 / 左侧主图` 都恢复为 `1x` 普通显示，不再对大图额外放大，优先保证网页与下载 PNG 的比例稳定性。
- 根据后续确认再次拆分网页与下载规则：网页端恢复“任一边超过 `1500px` 即按两倍显示”的原始设定，保留详情页内大图可读性；PNG 导出端则不再沿用图库大图的双倍冻结尺寸，而是将 `図鑑 / Skin` 图片按普通 `object-contain` 边界输出，优先避免下载结果出现比例失调或过度撑满栏位。
- 根据最新确认继续细分 PNG 图库导出：`図鑑 / Skin` 中任一边超过 `1500px`、且页面上本来已按两倍显示的图片，下载 PNG 时也恢复冻结当前渲染尺寸，确保导出结果维持两倍显示；同时普通图库图仍保留 `object-contain` 留白策略，避免所有图片一律硬填满栏位导致比例失调。
- 修正大图两倍显示的比例源头问题：此前 `図鑑 / Skin` 大图样式会分别对宽、高独立截断上限，可能在页面与 PNG 中把同一张细长图算成更矮更宽；现已改为按单一缩放系数统一计算宽高，在遵守最大显示边界的同时保持原始纵横比。
- 修正 `data/character-details.json` 中被 URL 编码的站内图片路径：将 `図鑑 / 他の参考画像 / 武器` 等 `%E5...` 形式路径还原为实际目录名，确保 JSON 与 `public/images` 的引用重新一致。
